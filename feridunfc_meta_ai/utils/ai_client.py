...
class AIClient:
    ...
    async def call_model(self, agent_role: str, prefer_costly: bool = False, prompt: str = "", system: str = ""):
        # Global mock kısa-devresi (CI/test)
    ...
    async def call_gemini(self, model: str, prompt: str, system: str | None = None):
        """
        Gemini çağrısı: Test/CI ortamı için anahtar yoksa gerçek HTTP yapma.
        """
        api_key = os.getenv("GOOGLE_API_KEY", "").strip()
        if not api_key or api_key == "<key>":
            return SimpleNamespace(content='{"output":"[mock] gemini disabled in tests"}', usage={"mock": True})

        # Gerçek çağrıyı çok basit bırakıyoruz; testlerde bu kola girilmemeli.
        ...
        return SimpleNamespace(content=text or '{"output":"(empty)"}', usage=data.get("usageMetadata", {}))

    # -------------------- Mock --------------------
    def _mock_response(self, agent_role: str, prompt: str):
        if agent_role == "architect":
            plan = {
                # iki isim birden: parser hangisini arıyorsa onu bulsun
                "sprint_title": "Sprint for: Basit TODO API",
                "title": "Sprint for: Basit TODO API",
                "sprint_goal": "Kullanıcı, görev ekle/listele uçlarıyla çalışan basit bir TODO API",
                "goal": "Kullanıcı, görev ekle/listele uçlarıyla çalışan basit bir TODO API",
                "weeks": [
                    {
                        "week_number": 1,
                        "tasks": [
